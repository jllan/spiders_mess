...
class Encoder(nn.Module):
    def __init__(self, ENC_TYPE, num_rnn_layers=1, rnn_hidden_size=160, dropout=0):
        ...
        self.layer3 = nn.Sequential(
            nn.Conv2d(64, 160, kernel_size=(5, 2), stride=(1, 1)),
            nn.BatchNorm2d(160),
            nn.ReLU()
        )
        self.gru = nn.GRU(160,rnn_hidden_size,num_rnn_layers,
                        batch_first=True,
                        dropout=dropout)
        self.linear = nn.Linear(160,rnn_hidden_size*num_rnn_layers)

        self.layer4 = nn.Conv1d(160, 256, kernel_size=3, stride=1, padding=1)
        self.layer5 = nn.Conv1d(160, 256, kernel_size=3, stride=1, padding=1)
        self.layer6 = nn.Conv1d(160, 256, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, x, hidden):
        if self.ENC_TYPE == 'CNNRNN':
            ...
            for layer in [self.layer4, self.layer5, self.layer6]:
                input = out
                out = layer(out)
                out = out[:, :160]*nn.functional.sigmoid(out[:, 160:])
                out = out + input
...
def predict(inputs, targets, encoder, decoder, use_cuda=False):
    encoder.train(False)
    decoder.train(False)
    batch_size = inputs.size()[0]
    max_len = targets.size()[1]
...
def predict2(inputs, targets, encoder, decoder, use_cuda=False):
    encoder.train(False)
    decoder.train(False)
    batch_size = inputs.size()[0]
    # max_len = targets.size()[1]
