...
class HotelDetailInfo(Base):
    ...
    highestPrice = Column(String(64), default=' ', nullable=False)
    ...
    returnMoney = Column(String(256), default=' ', nullable=False)
    hotelId = Column(String(20), ForeignKey('hotelBasicInfo.id', ondelete='CASCADE', onupdate='CASCADE'))

# 初始化数据库连接:
engine = create_engine('mysql+mysqlconnector://root:cbj@localhost:3306/jinjiang')
# 创建DBSession类型:
